...
function itEvaluates(name: string, fn: Func) {
  ...
  const index = expressions.indexOf(name);
  ...
  if (index != -1) {
    ...
    it(testName, function () {
      assert.fail(null, null, `Expression "${name}" either is not a core expression or has already been tested`);
    });
  }
}

after("Tested all expressions", function () {
  assert.deepEqual(expressions, [], "Not all expressions have been tested");
});

describe("Core expressions", function () {
